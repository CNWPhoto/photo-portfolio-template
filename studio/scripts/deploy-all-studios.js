...
// Directories NOT to copy into a scratch. Everything else in studio/ comes
// along automatically — the previous hand-written allowlist
// (schemaTypes, components, static) silently went stale the moment
// sanity.config.js grew imports of ./presentation, ./structure and ./lib, and
// every parallel deploy failed with "sanity.config.js: Module not found",
// naming the importer rather than the file actually missing. An excludelist
// fails safe: a new directory is copied by default instead of being forgotten.
const SCRATCH_DIR_EXCLUDES = new Set([
  'node_modules', // symlinked to the real one below
  'dist', // build output — regenerated by the deploy
  '.sanity', // runtime cache, per-deploy
  'scripts', // not needed to build a Studio
])
const scratchDirs = () =>
  fs
    .readdirSync(STUDIO_DIR, {withFileTypes: true})
    .filter((e) => e.isDirectory() && !SCRATCH_DIR_EXCLUDES.has(e.name))
    .map((e) => e.name)
